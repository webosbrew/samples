/*
 * Playing an H.264 + AAC pair through starfish-media-pipeline on webOS 5 and newer.
 *
 * The whole of it:
 *
 *   1. bring up SDL and a fullscreen, transparent window
 *   2. ask SDL for an exported (punch-through) window and remember its id
 *   3. Load() the pipeline with a BUFFERSTREAM payload carrying that id
...
int main(int argc, char *argv[]) {
    ...
    if (app_id == NULL) {
        app_id = "org.webosbrew.sample.media.smp.webos5";
    ...
    sdl_shell shell;
    if (!sdl_shell_preinit() || !sdl_shell_open_window(&shell, "SMP webOS 5+")) {
        return 1;
    }
    ...
    exported_window window;
    exported_window_init(&window, &shell, opts.width, opts.height);

    smp_video_plane plane;
    exported_window_plane(&window, &plane);
...
teardown:
    ...
    exported_window_destroy(&window);
    sdl_shell_close(&shell);
    es_file_close(video);
    es_file_close(audio);
    return exit_code;
