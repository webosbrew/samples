/*
 * Playing H.264 plus audio through NDL DirectMedia, on webOS 3.5 and newer.
 *
 * The whole of it:
 *
 *   1. bring up SDL and a fullscreen, transparent window
 *   2. NDL_DirectMediaInit, declare foreground, open the decoders, set the video area
 *   3. feed access units in presentation order, in real time
 *   4. close the decoders and NDL_DirectMediaQuit
 *
 ...
 * alone.
 *
 * This one main.c builds twice, against two incompatible versions of the same API - see
 * directmedia.h. The only thing it has to know about the difference is which audio file to
 * open, because v2 dropped AAC: directmedia_wants_pcm_audio() answers that, and everything
 * else is behind the facade.
 */
#include <limits.h>
#include <stdio.h>
#include <stdlib.h>
...
/* Must match the -ar / -ac that assets/make-sample.sh passes to ffmpeg for sample.pcm. */
...
int main(int argc, char *argv[]) {
    ...
    if (app_id == NULL) {
        app_id = "org.webosbrew.sample.media.ndl.directmedia";
    ...
    sdl_shell shell;
    if (!sdl_shell_preinit() || !sdl_shell_open_window(&shell, "NDL DirectMedia")) {
        return 1;
    }
    ...
    directmedia *player = directmedia_create(app_id);
    if (player == NULL) {
    ...
    /* Neither version has a way to say "that was the last buffer". v2's load callback does
     * carry an end-of-stream event, but with a buffer-stream source nothing ever triggers
     * it - observed on webOS 10.3.1, where the callback reports LOADCOMPLETED and PLAYING
     * and then nothing until Unload. So for both versions this timeout is the normal exit,
     * and it is only safe because the feed loop paced in real time: the last frame's
     * presentation moment has already passed by the time we get here. */
    if (exit_code == 0) {
        int64_t deadline = pacer_now_ns() + DRAIN_TIMEOUT_NS;
        while (!directmedia_ended(player) && pacer_now_ns() < deadline &&
               sdl_shell_pump(&shell)) {
            pacer_sleep_ns(20 * 1000 * 1000);
        }
        fprintf(stderr, "[main] %s\n", directmedia_ended(player)
                                                ? "drained on end of stream"
                                                : "drain timeout, closing");
    }

teardown:
    directmedia_close(player);
    directmedia_destroy(player);
    sdl_shell_close(&shell);
    es_file_close(video);
    es_file_close(audio);
    return exit_code;
