...
Each sample has its own launcher icon so they can be told apart on the home screen -
`assets/icons/<name>.png`, generated by `assets/make-icons.py` from the Material 500
palette and grouped by family: blues for starfish, greens for NDL, orange for LGNC. The
matching `iconColor` goes into `appinfo.json`. Re-run the script after adding a sample; the
PNGs are committed so the build needs no Python.
